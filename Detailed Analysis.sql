...
----avg profit per user (3 tables)
SELECT 
    AVG(profit_per_user) AS avg_profit_per_user
FROM (
    SELECT 
        o.user_id,
        SUM(oi.price_usd) - SUM(oi.cogs_usd) - COALESCE(SUM(r.refund_amount_usd), 0) AS profit_per_user
    FROM 
        orders o
    JOIN 
        order_items oi ON o.order_id = oi.order_id
...
SELECT 
    A.pageview_url,
    (SUM(CASE WHEN session_counts.page_count = 1 THEN 1 ELSE 0 END) * 100.0) / COUNT(*) AS Bounce_rate
FROM website_pageviews AS A
JOIN website_sessions AS B 
...
JOIN (
    ...
    ON A.website_session_id = session_counts.website_session_id
WHERE 
    B.utm_campaign = 'nonbrand' 
    AND B.utm_source = 'gsearch' 
    AND B.created_at < '2012-07-28' 
    AND A.pageview_url IN ('/lander-1', '/home')
...
MonthlyPurchases AS (
    SELECT 
        c.cohort_month,
        DATEADD(MONTH, DATEDIFF(MONTH, 0, o.created_at), 0) AS purchase_month,
        COUNT(DISTINCT o.user_id) AS customer_count
    FROM Cohort c
    JOIN orders o ON c.user_id = o.user_id
    GROUP BY c.cohort_month, DATEADD(MONTH, DATEDIFF(MONTH, 0, o.created_at), 0)
),
CohortMatrix AS (
    SELECT 
        cohort_month,
    ...
    WHERE DATEDIFF(MONTH, cohort_month, purchase_month) BETWEEN 0 AND 11
)
SELECT 
    FORMAT(cohort_month, 'MMM-yy') AS Cohort,
    SUM(CASE WHEN month_offset = 0 THEN customer_count ELSE 0 END) AS "Month 0",
    SUM(CASE WHEN month_offset = 1 THEN customer_count ELSE 0 END) AS "Month 1",
    SUM(CASE WHEN month_offset = 2 THEN customer_count ELSE 0 END) AS "Month 2",
    SUM(CASE WHEN month_offset = 3 THEN customer_count ELSE 0 END) AS "Month 3",
    SUM(CASE WHEN month_offset = 4 THEN customer_count ELSE 0 END) AS "Month 4",
    SUM(CASE WHEN month_offset = 5 THEN customer_count ELSE 0 END) AS "Month 5",
    SUM(CASE WHEN month_offset = 6 THEN customer_count ELSE 0 END) AS "Month 6",
    SUM(CASE WHEN month_offset = 7 THEN customer_count ELSE 0 END) AS "Month 7",
    SUM(CASE WHEN month_offset = 8 THEN customer_count ELSE 0 END) AS "Month 8",
    SUM(CASE WHEN month_offset = 9 THEN customer_count ELSE 0 END) AS "Month 9",
    SUM(CASE WHEN month_offset = 10 THEN customer_count ELSE 0 END) AS "Month 10",
    SUM(CASE WHEN month_offset = 11 THEN customer_count ELSE 0 END) AS "Month 11"
FROM CohortMatrix
...
WITH SessionClassification AS (
    SELECT
        CASE
            WHEN utm_source != 'null' AND utm_campaign = 'brand' THEN 'Paid Search Brand'
            WHEN utm_source != 'null' AND utm_campaign != 'brand' THEN 'Paid Search Non-Brand'
            WHEN utm_source = 'null' AND http_referer = 'null' THEN 'Direct Type-In'
            WHEN utm_source = 'null' AND http_referer != 'null' THEN 'Organic Search'
        END AS session_type,
        FORMAT(created_at, 'yyyy-MM') AS month  -- Extract the month in YYYY-MM format
    FROM website_sessions
),
SessionCounts AS (
    SELECT
    ...
    GROUP BY session_type, month
),
PaidNonBrandCount AS (
    SELECT
        month,
        session_count
    FROM SessionCounts
    WHERE session_type = 'Paid Search Non-Brand'
)
SELECT
    sc.month AS "Month",
    sc.session_type AS "Session Type",
    sc.session_count AS "Session Count",
    (sc.session_count * 100.0 / pnb.session_count) AS "Percentage of Paid Search Non-Brand"
FROM SessionCounts sc
JOIN PaidNonBrandCount pnb ON sc.month = pnb.month  -- Join on the month to get the Paid Search Non-Brand count by month
WHERE sc.session_type IN ('Organic Search', 'Direct Type-In', 'Paid Search Brand')
ORDER BY sc.month, sc.session_type;
...
-------------OR
        ...
        SELECT T1.pageview_url,Single_click_sessions  *100/Total_sessions_cnt AS Bounce_rate
        FROM (
               SELECT pageview_url, COUNT(*) AS Total_sessions_cnt FROM website_pageviews AS A
			    JOIN website_sessions AS B
		       ON A.website_session_id= B.website_session_id
               WHERE utm_campaign= 'nonbrand' AND utm_source= 'gsearch' AND
			         A.created_at < '2012-07-28' AND pageview_url IN ('/lander-1', '/home')
               GROUP BY pageview_url
               ) AS T1
         JOIN
        (
        SELECT pageview_url, COUNT(*) AS Single_click_sessions FROM website_pageviews AS A
		 JOIN website_sessions AS B
		ON A.website_session_id= B.website_session_id
        WHERE A.created_at < '2012-07-28' AND pageview_url IN ('/lander-1', '/home') AND
		      utm_campaign= 'nonbrand' AND utm_source= 'gsearch' AND
              a.website_session_id IN (
...
-- First, create a temporary table or Common Table Expression (CTE) for orders
/**********************************************************************************
 Analyze Conversion Funnel Tests for /billing vs. /billing-2 pages: 
 Compare traffic and billing-to-order conversion rates of both pages without using JOIN.
**********************************************************************************/
...
select session_month,
	 ...
	 SUM(CASE WHEN traffic_source = 'paid_nonbrand' THEN session_count END) 
		...
		from cte2
		/*partition by session_month is needed because we are comparing paid nonbrand session in a month with other
		traffic sources in that month. If we dont do that it will calculated for all rows (traffic sources in other months also))
		we restrain the rows to compare using partition by) In question 23 , we dont use partition by and we can see the difference */
	...
	----------- OR
		with flagged_sessions as
			...
			case when utm_source ='Null' and http_referer !='Null' then 'organic search'
			 when utm_source ='Null' and http_referer ='Null' then 'direct type'
...
session_count as (
			select format(created_at,'yyyy-MM') as session_month,
			traffic_source,
			count(website_session_id) as session_count
			from flagged_sessions
			where traffic_source is not null 
			group by format(created_at,'yyyy-MM') ,traffic_source) ,


paid_nonbrand_session as
			(select format(created_at,'yyyy-MM') as session_month,
			count(case when utm_campaign ='nonbrand' then 1 end) as paid_nonbrand_count
			from  website_data
			group by format(created_at,'yyyy-MM'))

select t1.*,session_count*100.0/paid_nonbrand_count as  percentage_of_paid_nonbrand
from session_count as t1 join paid_nonbrand_session as t2
on t1.session_month=t2.session_month
...
WITH hourly_sessions AS (
    SELECT
        DATEdiff(DAY,'2013-09-15', created_at) AS day_num,
    ...
    WHERE
        ...
        AND DATENAME(WEEKDAY, created_at) NOT IN ('Saturday', 'Sunday')
    GROUP BY
        DATEdiff(DAY,'2013-09-15', created_at),
        DATEPART(HOUR, created_at)
)
SELECT
    hour_of_day,
    AVG(session_count * 1.0) AS avg_sessions_per_hour
FROM
    hourly_sessions
GROUP BY
    hour_of_day
...
GROUP BY 
    ...
    DATEPART(WEEKDAY, created_at) -- Ensures correct ordering by day of the week
ORDER BY 
    DATEPART(WEEKDAY, created_at); -- datepart gives number instead of string compared to datename.
									--So monday-1, Tuesday -2 etc. we are ordering by this number ascending
...
---Flagging method
	WITH pageviews AS (
    -- Filter data for the baseline and post-launch time periods
    SELECT
        website_session_id,
        pageview_url,
        CASE
            WHEN created_at < '2013-01-06' AND created_at >= '2012-10-06' THEN 'Baseline'
            WHEN created_at >= '2013-01-06' AND created_at < '2013-04-06' THEN 'Post Launch'
        END AS time_period
    FROM website_pageviews
),
session_pageviews AS (
    -- Count the number of pageviews per session in each time period
    SELECT
        time_period,
        website_session_id,
        COUNT( pageview_url) AS pageview_count
...
sessions_aggregated AS (
    -- Aggregate sessions with more than 1 pageview and more than 2 pageviews
    SELECT
        time_period,
        COUNT(CASE WHEN pageview_count > 1 THEN 1 END) AS sessions_more_than_1_pageview,
        COUNT(CASE WHEN pageview_count > 2 THEN 1 END) AS sessions_more_than_2_pageview
...
-- Final output: Add the CTR calculation
SELECT
    time_period,
    sessions_more_than_1_pageview,
    sessions_more_than_2_pageview,
   (sessions_more_than_2_pageview * 100.0) / sessions_more_than_1_pageview
 ...
 ****************************************************************************************/
	DECLARE @columns NVARCHAR(MAX), @sql NVARCHAR(MAX); --We save both list of comma seperated columns and sql query

-- Generate a comma-separated list of product names dynamically
SELECT @columns = STRING_AGG(QUOTENAME(product_name), ',')
FROM (SELECT DISTINCT product_name FROM order_data) AS product_list;

-- Construct the pivot SQL dynamically
SET @sql = '
WITH refund_data AS (
    SELECT 
        FORMAT(order_created_at, ''yyyy-MM'') AS year_month,
        product_name,
        SUM(refund_amount_usd) * 100.0 / NULLIF(SUM(price_usd), 0) AS refund_rate
    FROM order_data
...
WITH refund_data AS (
    ...
    FROM 
        order_data
    GROUP BY 
        FORMAT(order_created_at, 'yyyy-MM'), product_name
)
SELECT * 
FROM refund_data
PIVOT (
...
WITH cte AS (
    SELECT 
		...
		CREATED_AT,
        ROW_NUMBER() OVER (PARTITION BY USER_ID ORDER BY created_at) AS row_num,
        LAG(created_at) OVER (PARTITION BY USER_ID ORDER BY created_at) AS previous_session
    FROM 
        website_sessions
    WHERE 
        YEAR(created_at) >= 2014
...
SELECT 
    MIN(DATEDIFF(day, previous_session, created_at)) AS min_time_between_sessions,
    MAX(DATEDIFF(day, previous_session, created_at)) AS max_time_between_sessions,
    AVG(DATEDIFF(day, previous_session, created_at) * 1.0) AS avg_time_between_sessions
FROM 
    cte
WHERE 
    row_num = 2  
